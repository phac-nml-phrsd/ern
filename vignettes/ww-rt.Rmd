...
- `ww.smooth`: the smoothed wastewater concentration over time
...
- `R`: the daily reproduction number, which we compute in an ensemble. Each run in the ensemble is computed by using `EpiEstim::estimate_R()` on an inferred daily incidence time series and a generation interval. Inferred daily incidence time series are drawn from the individual realizations used to produce the `inc` data frame described above. Generation interval distributions are drawn from the specified family (`dist.gi`). The $\mathcal{R}_t$ ensemble is then summarized for each date with a mean, lower CI bound, and upper CI bound. The CI width can be specified by the user in `prm.R`.
