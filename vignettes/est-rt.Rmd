...
```{r chunck options, include = FALSE}
...
A key feature of clinical surveillance data is that they often are aggregated,
typically on a weekly basis (e.g., for respiratory infections).

Hence, a critical step is to disaggregate this observation data into daily 
observations. Several methods are implemented, labelled `linear`, `renewal` and
`smooth`. 

The `linear` method disaggregate observations into a daily piecewise linear function.
The `renewal` method fits a simple epidemic model implemented with a "renewal equation" 
to then estimate the daily incidence. 
The `smooth` method disaggregate observations into a smooth function using 
convex optimization.

For most applications, we suggest using the `smooth` method.
...
```{r init-prms for clinical data}
# settings for daily report inference
prm.daily = list(
  method            = "smooth",
  smooth.input      = TRUE, 
  smooth.input.span = 0.3
)
...
```{r Rt clinical estimation, warning=FALSE}
