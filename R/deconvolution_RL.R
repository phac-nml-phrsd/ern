...
#' @param times A numeric vector of times corresponding to the entries in observed.
...
deconvolution_RL <- function(
  ...
  if(length(p_delay)<length(observed)){
    # Pad the end of p_delay with 0s so it's the same length as the observations
    p_delay <- c(p_delay,
